...
DECLARE
  ...
  v_workdays INT; v_submitted_weekdays INT; v_missing INT;
BEGIN
  ...
  SELECT
    ...
    COUNT(*) FILTER (WHERE EXTRACT(ISODOW FROM work_date) < 6)
  INTO v_days, v_hours, v_late, v_absent, v_ot, v_leave, v_submitted_weekdays
  FROM public.daily_time_reports
  WHERE employee_id = _employee AND cutoff_id = _cutoff;

  v_missing := GREATEST(v_workdays - v_submitted_weekdays, 0);
...
-- Recompute all existing submissions
DO $$
DECLARE r RECORD;
BEGIN
  FOR r IN SELECT employee_id, cutoff_id FROM public.dtr_cutoff_submissions WHERE cutoff_id IS NOT NULL LOOP
