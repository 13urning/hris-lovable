...
import {
  ...
  fetchEvaluationsByPeriod,
  fetchKpiScoresByEvalId,
  fetchBehavioralScoresByEvalId,
  fetchAllProfiles,
  fetchActiveKpiTemplates,
  fetchActiveBehavioralCompetencies,
  insertEvaluationPeriod,
  updateEvaluationPeriodStatus,
  insertEvaluationsForPeriod,
  insertKpiScores,
  insertBehavioralScores,
  updateKpiHrScore,
  updateBehavioralGhScore,
  approveEvaluation,
...
export const Route = createFileRoute("/_authenticated/_admin/performance-admin")({
...
type BehavioralScore = {
  ...
  employee_accomplishments: string | null;
  employee_rating: number | null;
  gh_rating: number | null;
  gh_comments: string | null;
...
const STATUS_COLORS: Record<string, string> = {
  ...
  active: "default",
  closed: "outline",
  pending_self_assessment: "secondary",
  self_assessed: "default",
  approved: "default",
};
...
function PeriodBadge({ s }: { s: string }) {
  ...
  return (
    <Badge
      variant={
        (STATUS_COLORS[s] ?? "secondary") as "default" | "secondary" | "outline" | "destructive"
      }
    >
      {labels[s] ?? s}
    </Badge>
...
function ScoreStars({ score, onChange }: { score: number | null; onChange?: (v: number) => void }) {
  return (
    <div className="flex gap-0.5">
      ...
      {score != null && (
        <span className="ml-1 text-sm text-muted-foreground">{Number(score).toFixed(1)}/5</span>
      )}
    </div>
  );
}
...
function PerformanceAdminPage() {
  ...
  const [kpiPatches, setKpiPatches] = useState<
    Record<string, { hr_score: number | null; hr_actual_value: number | null; hr_comments: string }>
  >({});
  const [behavioralPatches, setBehavioralPatches] = useState<
    Record<string, { gh_rating: number | null; gh_comments: string }>
  >({});
  ...
  const { data: evaluations = [] } = useQuery({
    ...
    queryFn: () =>
      fetchEvaluationsByPeriod({ data: { periodId: activePeriod!.id } }) as Promise<Evaluation[]>,
  });
  ...
  const { data: evalScores } = useQuery({
    ...
    queryFn: () =>
      fetchKpiScoresByEvalId({ data: { evaluationId: reviewingEval!.id } }) as Promise<KpiScore[]>,
  });
  ...
  const { data: behavioralScores } = useQuery({
    ...
    enabled: !!reviewingEval,
    queryFn: () =>
      fetchBehavioralScoresByEvalId({ data: { evaluationId: reviewingEval!.id } }) as Promise<
        BehavioralScore[]
      >,
  });
  ...
  const createPeriod = useMutation({
    mutationFn: async () => {
      await insertEvaluationPeriod({
        data: { ...periodForm, created_by: user!.id, status: "draft" },
      });
    },
    onSuccess: () => {
  ...
  const generateEvaluations = useMutation({
    mutationFn: async (periodId: string) => {
      ...
      if (toCreate.length === 0) {
        toast.info("All employees already have evaluations");
        return;
      }

      const inserted = await insertEvaluationsForPeriod({
        data: {
          evaluations: toCreate.map((emp) => ({
            employee_id: emp.id,
            period_id: periodId,
      ...
      for (const row of inserted) {
        ...
        if (!emp) continue;
        for (const k of (kpiTemplates as KpiTpl[]).filter(
          (k) =>
            k.team === emp.department && (k.designation === null || k.designation === emp.position),
        )) {
          kpiPayloads.push({
  ...
  const livePreview = (() => {
    ...
    const finalBeh = behavioralScores
      .map((b) => {
    ...
    const behAvg =
      finalBeh.length > 0 ? finalBeh.reduce((s, v) => s + v, 0) / finalBeh.length : null;
  ...
  const statusCounts = {
    ...
    pending_self_assessment: evaluations.filter((e) => e.status === "pending_self_assessment")
      .length,
  };

  // Filtered list
  const filteredEvaluations = ratingFilter
    ? evaluations.filter((e) => e.overall_rating === ratingFilter || e.status === ratingFilter)
    : evaluations;
  ...
  const handleExportEvaluations = () => {
    exportRowsToCSV(
      ...
      [
        ...
        {
          header: "KPI Score",
          value: (e) => (e.kpi_score != null ? Number(e.kpi_score).toFixed(2) : ""),
        },
        {
          header: "Behavioral Score",
          value: (e) => (e.behavioral_score != null ? Number(e.behavioral_score).toFixed(2) : ""),
        },
        {
          header: "Overall Score",
          value: (e) => (e.overall_score != null ? Number(e.overall_score).toFixed(2) : ""),
        },
    ...
    <div className="space-y-6">
      <div className="flex items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            ...
            <strong>Group Head reviews, scores, and approves</strong>. Final rating is computed from
            the KPI × Behavioral matrix.
          </p>
        </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                  ...
                  <Button
                    ...
                    onClick={(e) => {
                      ...
                      confirm.ask({
                        ...
                        onConfirm: () =>
                          updatePeriodStatus.mutateAsync({ id: p.id, status: "active" }),
                      });
                    }}
                  >
                    Activate
                  ...
                  <Button
                    ...
                    onClick={(e) => {
                      ...
                      confirm.ask({
                        ...
                        destructive: true,
                        onConfirm: () =>
                          updatePeriodStatus.mutateAsync({ id: p.id, status: "closed" }),
                      });
                    }}
                  >
                    Close
        ...
        {periods.length === 0 && (
          <p className="col-span-full text-sm text-muted-foreground">
            No evaluation periods yet. Create one to get started.
          </p>
        )}
      </div>
              ...
              {[
                {
                  ...
                  count: evalsByStatus.pending.length,
                  color: "text-muted-foreground",
                },
                {
                  label: "Awaiting Group Head",
                  count: evalsByStatus.self_assessed.length,
                  color: "text-yellow-600",
                },
                {
                  label: "Approved",
                  count: evalsByStatus.approved.length,
              ...
              <div className="flex flex-wrap gap-2 mb-4">
                ...
                {statusCounts.pending_self_assessment > 0 && (
                  ...
                  >
                    ...
                    <span className="rounded-full bg-warning/20 px-1.5 py-0.5 text-[10px] font-bold">
                      {statusCounts.pending_self_assessment}
                    </span>
                  </button>
                )}
                {statusCounts.self_assessed > 0 && (
                  <button
                    onClick={() =>
                      setRatingFilter(ratingFilter === "self_assessed" ? null : "self_assessed")
                    }
                    className={`flex items-center gap-1.5 rounded-full border px-3 py-1 text-xs font-medium transition-colors ${
                ...
                {statusCounts.approved > 0 && (
                  ...
                  >
                    Approved
                    <span className="rounded-full bg-success/20 px-1.5 py-0.5 text-[10px] font-bold">
            ...
            <Tabs defaultValue="self_assessed">
              <TabsList>
                <TabsTrigger value="pending">
                  Pending Self-Assess ({evalsByStatus.pending.length})
                </TabsTrigger>
                <TabsTrigger value="self_assessed">
                  Awaiting Your Review ({evalsByStatus.self_assessed.length})
                </TabsTrigger>
                <TabsTrigger value="approved">
                  Approved ({evalsByStatus.approved.length})
                </TabsTrigger>
              </TabsList>
                  ...
                  {evalsByStatus[tab].length === 0 ? (
                    ...
                    <div className="space-y-2">
                      {evalsByStatus[tab].map((ev) => {
                        const colors = ev.overall_rating
                          ? RATING_COLORS[ev.overall_rating as keyof typeof RATING_COLORS]
                          : null;
                        return (
                            ...
                            <div className="flex items-center justify-between px-4 py-3">
                              ...
                              <div className="flex items-center gap-3">
                                {ev.overall_rating && colors && (
                                  <span
                                    className={`text-xs px-2 py-1 rounded-full border font-medium ${colors.bg} ${colors.text} ${colors.border}`}
                                  >
                                    {ev.overall_rating}
                                  </span>
                                )}
                                  ...
                                  <div className="text-right text-xs">
                                    <p>
                                      ...
                                      <span className="font-semibold">
                                        {Number(ev.kpi_score).toFixed(2)}
                                      </span>
                                    </p>
                                    <p>
                                      Beh{" "}
                                      <span className="font-semibold">
                                        {Number(ev.behavioral_score).toFixed(2)}
                                      </span>
                                    </p>
                                  </div>
                                )}
                                ...
                                {tab === "self_assessed" && (
                                  <Button size="sm" onClick={() => openReview(ev)}>
                                    Review & Approve
                                  </Button>
                                )}
                                {tab === "approved" && (
      ...
      <Dialog open={showPeriodForm} onOpenChange={setShowPeriodForm}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>New Evaluation Period</DialogTitle>
          </DialogHeader>
        ...
        <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-6 py-2">
            ...
            {livePreview && livePreview.overall && (
              <div
                className={`rounded-lg border-2 p-4 ${RATING_COLORS[livePreview.overall].bg} ${RATING_COLORS[livePreview.overall].border}`}
              >
                <div className="flex items-center justify-between gap-4 mb-2">
                  <div>
                    <p className="text-xs uppercase tracking-wide text-muted-foreground">
                      {reviewingEval?.status === "approved"
                        ? "Overall Performance Rating"
                        : "Preview — Overall Rating"}
                    </p>
                    <p
                      className={`font-display text-2xl mt-0.5 ${RATING_COLORS[livePreview.overall].text}`}
                    >
                      {livePreview.overall}
                    </p>
                  </div>
                  <div className="text-right text-sm space-y-0.5">
                    <p>
                      ...
                      <strong>{livePreview.kpi?.toFixed(2) ?? "—"}</strong>
                    </p>
                    <p>
                      <span className="text-muted-foreground">Behavioral</span>{" "}
                      <strong>{livePreview.behavioral?.toFixed(2) ?? "—"}</strong>
                    </p>
                  </div>
                </div>
                <p className="text-xs text-muted-foreground">
                  {RATING_DESCRIPTIONS[livePreview.overall]}
                </p>
              </div>
            )}
            ...
            <div className="space-y-3">
              ...
              {evalScores && evalScores.length > 0 ? (
                    ...
                    <div key={score.id} className="rounded-lg border p-3 space-y-2">
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <p className="text-xs text-muted-foreground">
                            Target: {score.kpi_target} {score.kpi_metric_unit} · Weight:{" "}
                            {score.kpi_weight}%
                          </p>
                        </div>
                      </div>
                      <div className="grid grid-cols-2 gap-3 text-sm">
                        <div className="rounded bg-secondary/40 p-2">
                          ...
                          {score.self_actual_value != null && (
                            <p className="text-xs mt-1">
                              Actual: {score.self_actual_value} {score.kpi_metric_unit}
                            </p>
                          )}
                          {score.self_comments && (
                        ...
                        <div className="rounded bg-accent/10 p-2">
                          ...
                          {!isEditable && score.hr_actual_value != null && (
                            <p className="text-xs mt-1">
                              Actual: {score.hr_actual_value} {score.kpi_metric_unit}
                            </p>
                          )}
                ...
                <p className="text-sm text-muted-foreground p-3 rounded border">
                  No KPIs assigned. The employee will only be scored on behavioral competencies.
                </p>
              )}
            </div>
            ...
            <div className="space-y-3">
              <div className="flex items-center gap-2 border-b pb-2">
                ...
                <p className="text-sm font-semibold uppercase tracking-wide">
                  Part II · Behavioral Competencies
                </p>
              </div>
              <p className="text-xs text-muted-foreground">
                Rate each: 5 = Consistently · 4 = Frequently · 3 = Sometimes · 2 = Seldom · 1 = Does
                Not Demonstrate. Final per-competency rating = average of employee + Group Head.
              </p>
              {behavioralScores && behavioralScores.length > 0 ? (
                  ...
                  return (
                    <div key={b.id} className="rounded-lg border p-3 space-y-2">
                      <div>
                        ...
                        <pre className="text-[11px] text-muted-foreground whitespace-pre-wrap font-sans mt-1">
                          {b.competency_indicators}
                        </pre>
                      </div>
                      {b.employee_accomplishments && (
                        <div className="rounded bg-secondary/30 p-2 text-xs">
                          <p className="uppercase tracking-wide text-muted-foreground mb-0.5">
                            Accomplishments / Critical Incidents (from employee)
                          </p>
                ...
                <p className="text-sm text-muted-foreground p-3 rounded border">
                  No behavioral competencies loaded.
                </p>
              )}
            </div>
            ...
            <div className="space-y-1.5">
              <Label>
                Group Head Notes{" "}
                {reviewingEval?.status === "self_assessed" && "(visible to the employee)"}
              </Label>
              {reviewingEval?.status === "self_assessed" ? (
                <Textarea
                  value={groupHeadNotes}
                  onChange={(e) => setGroupHeadNotes(e.target.value)}
                  rows={3}
                  placeholder="Overall feedback for the employee…"
                />
              ) : (
                <div className="rounded-lg border bg-secondary/30 p-3 text-sm min-h-[60px]">
                  {reviewingEval?.group_head_notes || (
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setReviewingEval(null)}>
              Close
            </Button>
            {reviewingEval?.status === "self_assessed" && (
              <Button
                onClick={() => reviewAndApprove.mutate()}
                disabled={reviewAndApprove.isPending}
...
function EvalScorePreview({ evalId }: { evalId: string }) {
  const { data: scores = [] } = useQuery({
    ...
    queryFn: () =>
      fetchKpiScoresByEvalId({ data: { evaluationId: evalId } }) as Promise<KpiScore[]>,
  });
  const { data: bScores = [] } = useQuery({
    queryKey: ["eval-behavioral-preview", evalId],
    queryFn: () =>
      fetchBehavioralScoresByEvalId({ data: { evaluationId: evalId } }) as Promise<
        BehavioralScore[]
      >,
  });
              ...
              {bScores.map((b) => (
                <tr key={b.id} className="border-t border-secondary/50">
                  ...
                  <td className="text-center font-medium">
                    {b.final_rating ? Number(b.final_rating).toFixed(1) : "—"}
                  </td>
                </tr>
              ))}
            </tbody>
