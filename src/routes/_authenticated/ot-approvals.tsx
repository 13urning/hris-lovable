...
import {
  getMyOTBudgets,
  getMyActualOTs,
  getApprovedOTBudgets,
  fetchMyPendingOTApprovals,
  fileOTBudgetRequest,
  fileActualOTHours,
  approveOTStep,
  rejectOTStep,
...
function StatusBadge({ status }: { status: OTRequest["status"] | "logged" }) {
  if (status === "logged") {
    return <span className="rounded bg-success/15 px-2 py-0.5 text-xs text-success">logged</span>;
  }
  const cls =
    status === "approved"
...
function OTApprovalsPage() {
  ...
  const selectedBudget = approvedBudgets?.find((b) => b.id === fileForm.pre_approved_id) ?? null;
  const approvedForSelected = selectedBudget ? (approvedHoursById[selectedBudget.id] ?? 0) : 0;
  const pendingForSelected = selectedBudget ? (pendingHoursById[selectedBudget.id] ?? 0) : 0;
  ...
  const handleExportPending = () => {
    exportRowsToCSV(
      ...
      [
        ...
        {
          header: "Type",
          value: (r) => (r.request_type === "pre_approved" ? "Budget" : "Filed hours"),
        },
        {
          header: "For",
          value: (r) =>
        ...
        { header: "Notes", value: (r) => r.review_notes ?? "" },
        {
          header: "Step",
          value: (r) =>
            `${Math.min(r.current_approver_index + 1, r.approver_chain.length)} of ${r.approver_chain.length}`,
        },
    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Request monthly OT budgets and file actual hours against approved budgets.
          </p>
        </div>
      </div>
        ...
        <Card className="border-primary/30">
          <CardHeader className="flex flex-row flex-wrap items-start justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-sm text-muted-foreground">
                Request approval for OT hours to be used next month. Once approved, these become
                your OT allocation and deduct as you log actual hours.
              </p>
            </div>
            {!nextMonthBudget && (
              <Button
              ...
              <div className="flex flex-wrap items-center gap-6">
                <div>
                  ...
                  <p className="text-2xl font-bold tabular-nums">
                    {nextMonthBudget.requested_hours}h
                  </p>
                </div>
                <div>
                  <p className="text-xs uppercase tracking-wide text-muted-foreground">Status</p>
                  <div className="mt-0.5">
                    <StatusBadge status={nextMonthBudget.status} />
                  </div>
                </div>
                      ...
                      <>
                        ...
                        {pending > 0 && (
                          <div>
                            ...
                            <p className="text-lg font-semibold tabular-nums text-warning-foreground">
                              {pending}h
                            </p>
                          </div>
                        )}
                        <div>
                          ...
                          <p
                            className={`text-lg font-semibold tabular-nums ${remaining === 0 ? "text-destructive" : "text-success"}`}
                          >
                            {remaining}h
                          </p>
                        </div>
        ...
        <Card>
          <CardHeader className="flex flex-row flex-wrap items-center justify-between gap-3">
            ...
            <Button
              ...
              title={!approvedBudgets?.length ? "No approved OT budgets available" : undefined}
            >
      ...
      <Dialog open={budgetDialogOpen} onOpenChange={setBudgetDialogOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle className="font-display text-2xl">Request OT Budget</DialogTitle>
          </DialogHeader>
      ...
      <Dialog open={fileDialogOpen} onOpenChange={setFileDialogOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle className="font-display text-2xl">File OT Hours</DialogTitle>
          </DialogHeader>
          ...
          <div className="space-y-4 py-2">
                ...
                {approvedBudgets?.map((b) => {
                  ...
                  return (
                    <option key={b.id} value={b.id}>
                      {formatMonth(b.target_month)} — {b.requested_hours}h ({used}h used{pendingTag}
            ...
            <div>
              ...
              <Input
                ...
                onChange={(e) => setFileForm({ ...fileForm, work_date: e.target.value })}
              />
            </div>
