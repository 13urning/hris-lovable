...
import {
  ...
  updateKpiSelfScore,
  updateBehavioralSelfScore,
  markEvaluationSelfAssessed,
...
function EvalBadge({ status }: { status: string }) {
  ...
  return (
    <Badge variant={color as "default" | "secondary" | "outline" | "destructive"}>{label}</Badge>
  );
}
...
function ScoreStars({ score, onChange }: { score: number | null; onChange?: (v: number) => void }) {
  return (
    <div className="flex items-center gap-0.5">
      ...
      {score != null && (
        <span className="ml-1.5 text-sm text-muted-foreground">{Number(score).toFixed(1)}/5</span>
      )}
    </div>
  );
}
...
function PerformancePage() {
  ...
  const [kpiPatches, setKpiPatches] = useState<
    Record<string, { self_score: number | null; self_actual_value: string; self_comments: string }>
  >({});
  const [behavioralPatches, setBehavioralPatches] = useState<
    Record<string, { employee_rating: number | null; employee_accomplishments: string }>
  >({});
  ...
  const { data: kpiScores = [] } = useQuery({
    ...
    queryFn: () =>
      fetchKpiScoresByEvalId({ data: { evaluationId: assessingEval!.id } }) as Promise<KpiScore[]>,
  });
  ...
  const { data: behavioralScores = [] } = useQuery({
    ...
    enabled: !!assessingEval,
    queryFn: () =>
      fetchBehavioralScoresByEvalId({ data: { evaluationId: assessingEval!.id } }) as Promise<
        BehavioralScore[]
      >,
  });
  ...
  const submitSelfAssessment = useMutation({
    mutationFn: async () => {
      ...
      for (const beh of behavioralScores) {
        ...
        if (patch) {
          await updateBehavioralSelfScore({
            data: {
              id: beh.id,
      ...
      await markEvaluationSelfAssessed({
        data: {
  ...
  const selfPreview = (() => {
    ...
    for (const s of kpiScores) {
      ...
      if (score != null) {
        weightedSum += score * s.kpi_weight;
        totalWeight += s.kpi_weight;
      }
    }
    ...
    const behAvg =
      behRatings.length > 0 ? behRatings.reduce((s, v) => s + v, 0) / behRatings.length : null;
    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          Each review has two parts — Part I (your role KPIs) and Part II (universal behavioral
          competencies). Your overall rating is determined by the KPI × Behavioral matrix.
        </p>
      </div>
            ...
            <Card key={ev.id} className="border-accent/30 bg-accent/5">
              ...
              <CardContent>
                <p className="text-sm text-muted-foreground mb-3">
                  Complete your self-assessment for both Part I (KPIs) and Part II (Behavioral
                  Competencies).
                </p>
            ...
            <Card key={ev.id}>
              ...
              <CardContent>
                <div className="flex items-center gap-3 text-sm text-muted-foreground">
                  ...
                  {ev.self_assessment_submitted_at
        ...
        <div className="space-y-3">
          ...
          {completed.map((ev) => {
            const colors = ev.overall_rating
              ? RATING_COLORS[ev.overall_rating as OverallRating]
              : null;
            return (
                ...
                <CardContent>
                  {ev.overall_rating && colors && (
                    <div className={`mb-4 rounded-lg border p-4 ${colors.bg} ${colors.border}`}>
                      ...
                      <p className={`font-display text-3xl mt-1 ${colors.text}`}>
                        {ev.overall_rating}
                      </p>
                      <p className="text-xs text-muted-foreground mt-1">
                        {RATING_DESCRIPTIONS[ev.overall_rating as OverallRating]}
                      </p>
                      <div className="mt-3 grid grid-cols-2 gap-3 text-sm">
                        <div className="rounded bg-background/60 p-2 text-center">
                          ...
                          <p className="font-display text-xl">
                            {ev.kpi_score != null ? Number(ev.kpi_score).toFixed(2) : "—"}
      ...
      <Dialog open={!!assessingEval} onOpenChange={(o) => !o && setAssessingEval(null)}>
        <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-6 py-2">
            ...
            {assessingEval?.status === "approved" &&
              assessingEval.overall_rating &&
              (() => {
                ...
                return (
                  <div className={`rounded-lg border-2 p-4 ${colors.bg} ${colors.border}`}>
                    ...
                    <p className={`font-display text-2xl mt-1 ${colors.text}`}>
                      {assessingEval.overall_rating}
                    </p>
                    <p className="text-xs text-muted-foreground mt-1">
                      {RATING_DESCRIPTIONS[assessingEval.overall_rating as OverallRating]}
                    </p>
                    <div className="mt-3 grid grid-cols-2 gap-3">
                      <div className="rounded bg-background/60 p-2 text-center text-sm">
                        ...
                        <p className="font-semibold text-lg">
                          {assessingEval.kpi_score != null
                            ? Number(assessingEval.kpi_score).toFixed(2)
                            : "—"}
                        </p>
                      </div>
                      <div className="rounded bg-background/60 p-2 text-center text-sm">
                        ...
                        <p className="font-semibold text-lg">
                          {assessingEval.behavioral_score != null
                            ? Number(assessingEval.behavioral_score).toFixed(2)
                            : "—"}
                        </p>
                      </div>
                    </div>
            ...
            {selfPreview?.overall && (
              <div
                className={`rounded-lg border-2 p-4 ${RATING_COLORS[selfPreview.overall].bg} ${RATING_COLORS[selfPreview.overall].border}`}
              >
                <div className="flex items-center justify-between gap-4 mb-2">
                  <div>
                    ...
                    <p
                      className={`font-display text-2xl mt-0.5 ${RATING_COLORS[selfPreview.overall].text}`}
                    >
                      {selfPreview.overall}
                    </p>
                  </div>
                  <div className="text-right text-sm space-y-0.5">
                    <p>
                      ...
                      <strong>{selfPreview.kpi?.toFixed(2) ?? "—"}</strong>
                    </p>
                    <p>
                      <span className="text-muted-foreground">Behavioral </span>
                      <strong>{selfPreview.behavioral?.toFixed(2) ?? "—"}</strong>
                    </p>
                  </div>
                </div>
                <p className="text-xs text-muted-foreground">
                  {RATING_DESCRIPTIONS[selfPreview.overall]}
                </p>
                {assessingEval?.status === "pending_self_assessment" && (
                  <p className="text-[11px] text-muted-foreground/70 mt-2">
                    Based on your current ratings. The final rating is determined after Group Head
                    review.
                  </p>
                )}
                  ...
                  return (
                    <div key={score.id} className="rounded-lg border p-4 space-y-3">
                      ...
                      <div>
                        ...
                        <ScoreStars
                          score={patch?.self_score ?? score.self_score}
                          onChange={
                            isEditable
                              ? (v) =>
                                  setKpiPatches((prev) => ({
                          ...
                          <div className="mt-2 grid grid-cols-2 gap-2">
                            <div>
                              <Label className="text-xs">
                                Actual Value ({score.kpi_metric_unit})
                              </Label>
                              <Input
                                ...
                                onChange={(e) =>
                                  setKpiPatches((prev) => ({
                                    ...
                                    [score.id]: {
                                      ...
                                      self_actual_value: e.target.value,
                                    },
                                  }))
                                }
                              />
                        ...
                        {!isEditable && score.self_score && (
                          <div className="mt-2 text-sm text-muted-foreground">
                            ...
                            {score.self_comments && (
                              <span className="italic">"{score.self_comments}"</span>
                            )}
                          </div>
                        )}
                      </div>
                      {score.hr_score != null && (
                        <div className="border-t pt-3">
                          <p className="text-xs uppercase tracking-wide text-muted-foreground mb-2">
                            Group Head Assessment (Liv Olarte)
                          </p>
            ...
            {behavioralScores.length > 0 && (
              <div className="space-y-3">
                ...
                {assessingEval?.status === "pending_self_assessment" && (
                  <p className="text-xs text-muted-foreground">
                    5 = Consistently · 4 = Frequently · 3 = Sometimes · 2 = Seldom · 1 = Does Not
                    Demonstrate. Add accomplishments / critical incidents to support your rating.
                  </p>
                )}
                  ...
                  return (
                    <div key={b.id} className="rounded-lg border p-4 space-y-3">
                      ...
                      <div>
                                  ...
                                  setBehavioralPatches((prev) => ({
                                    ...
                                    [b.id]: {
                                      ...
                                      employee_accomplishments:
                                        prev[b.id]?.employee_accomplishments ??
                        ...
                        {currentRating != null && (
                          <p className="text-xs text-muted-foreground mt-1">
                            {RATING_FREQ[Math.round(currentRating)] ?? ""}
                          </p>
                        )}
                        {isEditable ? (
                          <div className="mt-2">
                            ...
                            <Textarea
                              ...
                              value={
                                patch?.employee_accomplishments ?? b.employee_accomplishments ?? ""
                              }
                              onChange={(e) =>
                                setBehavioralPatches((prev) => ({
                                  ...prev,
                                  [b.id]: {
                                    employee_rating:
                                      prev[b.id]?.employee_rating ?? b.employee_rating,
                      ...
                      {b.final_rating != null && (
                        <div className="rounded bg-accent/10 px-3 py-2 text-sm flex items-center justify-between">
                          ...
                          <span className="font-semibold">
                            {Number(b.final_rating).toFixed(2)} / 5
                          </span>
                        </div>
                      )}
                    </div>
            ...
            {assessingEval?.status !== "pending_self_assessment" &&
              assessingEval?.group_head_notes && (
          ...
          <DialogFooter>
            ...
            {assessingEval?.status === "pending_self_assessment" && (
              <Button
                onClick={() => submitSelfAssessment.mutate()}
                disabled={
                  submitSelfAssessment.isPending ||
                  (kpiScores.length === 0 && behavioralScores.length === 0)
                }
              >
