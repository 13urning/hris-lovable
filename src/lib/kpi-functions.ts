...
export const fetchKpiTemplates = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context }) => {
    ...
    const { rows } = await pool.query(`SELECT * FROM kpi_templates ORDER BY team, title`);
...
export const upsertKpiTemplate = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    if (id) {
      ...
      await pool.query(`UPDATE kpi_templates SET ${sets} WHERE id = $${vals.length}`, vals);
    } else {
      ...
      await pool.query(`INSERT INTO kpi_templates (${quotedCols}) VALUES (${placeholders})`, vals);
    }
  });
